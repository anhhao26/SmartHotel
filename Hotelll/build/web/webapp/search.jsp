...
<head>
    ...
    <style>
        ...
        .input-elegant { background: #FFFFFF; border: 1px solid rgba(184, 154, 108, 0.2); color: #2C2722; transition: all 0.4s cubic-bezier(0.165, 1, 0.3, 1); }
...
<body class="font-sans antialiased min-h-screen flex flex-col items-center justify-center p-6 relative">
    ...
    <%
        ...
        if (roomIdStr != null && !roomIdStr.isEmpty()) {
            ...
            try {
                // Chỉ lấy những ngày của đơn đã thanh toán hoặc đang ở
                bookedList = em.createQuery("SELECT b FROM Booking b WHERE b.room.roomNumber = :rm AND b.status IN ('Confirmed', 'Checked-in')", Booking.class)
            ...
            <form action="${pageContext.request.contextPath}/booking" method="post" onsubmit="this.querySelector('button[type=submit]').innerHTML='ĐANG XỬ LÝ...'; this.querySelector('button[type=submit]').disabled=true;" class="space-y-10">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <div class="space-y-1">
                        ...
                        <div class="relative">
                            <input type="text" id="checkIn" class="w-full h-16 px-6 input-elegant rounded-xl font-bold text-hotel-text text-sm tracking-widest border-hotel-gold/10" name="checkIn" placeholder="Chọn ngày" required>
                        </div>
                    </div>
                    <div class="space-y-1">
                        <label class="label-premium ml-1">Thời điểm trả phòng (Check-Out)</label>
                        <div class="relative">
                            <input type="text" id="checkOut" class="w-full h-16 px-6 input-elegant rounded-xl font-bold text-hotel-text text-sm tracking-widest border-hotel-gold/10" name="checkOut" placeholder="Chọn ngày" required>
                        </div>
                    </div>
                </div>
            ...
            const checkInPicker = flatpickr("#checkIn", {
                ...
                onChange: function(selectedDates, dateStr, instance) {
                    if (selectedDates.length > 0) {
                        // Khách chọn Check-in xong -> tự đẩy minDate của Check-out lên ngày hôm sau
                        let minOut = new Date(selectedDates[0]);
                        minOut.setDate(minOut.getDate() + 1);
                        checkOutPicker.set("minDate", minOut);
                        
                        // Nếu ngày Check-out đã chọn trước đó bị lỗi logic (nhỏ hơn ngày Check-in), xóa đi
